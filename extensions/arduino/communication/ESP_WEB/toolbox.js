/* eslint-disable func-style */
/* eslint-disable max-len */
/* eslint-disable require-jsdoc */
function addToolbox () {
    const WEB_ICO = 'data:image/svg+xml;base64,';
    return `
<category name="%{BKY_WEB_CATEGORY}" id="WEB_CATEGORY" colour="#42CCFF" secondaryColour="#42CCFF" iconURI="${WEB_ICO}">
    <block type="esp32_web_page_init">
        <statement name="DO">
            <block type="make_arduino_comment_text_1">
                <field name="data">要插入的网页元素放置在这里</field>
            </block>
        </statement>
    </block>
    <block type="esp32_web_page_add">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="name">
            <shadow type="text">
                <field name="TEXT">displayName</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_addTemperatureCard">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="name">
            <shadow type="text">
                <field name="TEXT">displayName</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_addSliderCard">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="name">
            <shadow type="text">
                <field name="TEXT">displayName</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_attachSliderChanged"></block>
    <block type="esp32_web_page_SliderChanged" ></block>
    <block type="esp32_web_page_Slider_id" >
        <value name="id">
            <shadow type="text">
                <field name="TEXT">1</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_attachButtonClick"></block>
    <block type="esp32_web_page_ButtonClick" ></block>
    <block type="esp32_web_page_Button_id" >
        <value name="id">
            <shadow type="text">
                <field name="TEXT">1</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_addLineChart">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="name">
            <shadow type="text">
                <field name="TEXT">displayName</field>
            </shadow>
        </value>
        <value name="y_axis_name">
            <shadow type="text">
                <field name="TEXT">Power KwH</field>
            </shadow>
        </value>
        <value name="x_axis_value">
            <shadow type="text">
                <field name="TEXT">x_axis</field>
            </shadow>
        </value>
        <value name="y_axis_value">
            <shadow type="text">
                <field name="TEXT">y_axis</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_updateLineChart">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="x_axis_value">
            <shadow type="text">
                <field name="TEXT">x_axis</field>
            </shadow>
        </value>
        <value name="y_axis_value">
            <shadow type="text">
                <field name="TEXT">y_axis</field>
            </shadow>
        </value>
    </block>
    <block type="esp32_web_page_update">
        <value name="id">
            <shadow type="math_number">
                <field name="NUM">1</field>
            </shadow>
        </value>
        <value name="value">
            <shadow type="text">
                <field name="TEXT">1</field>
            </shadow>
        </value>
    </block>
    <block type="web_serial_init"></block>
    <block type="web_print_data_32">
        <value name="web_printing">
            <shadow type="text">
                <field name="TEXT">0</field>
            </shadow>
        </value>
    </block>
    <block type="web_receive_data_32"></block>
</category>
`;
}

exports = addToolbox;
