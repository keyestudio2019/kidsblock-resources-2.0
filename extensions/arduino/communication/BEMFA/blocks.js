/* eslint-disable func-style */
/* eslint-disable max-len */
/* eslint-disable require-jsdoc */
function addBlocks (Blockly) {
    const BEMFA_COLOR = '#5b67a5'
    const BEMFA_ICO = 'data:image/png;base64,';

    //巴法云主题订阅
    Blockly.Blocks.tcp_device_cloud_subscription = {
        init: function () {
            this.jsonInit({
                message0: '%1',
                message1: Blockly.Msg.tcp_device_cloud_subscription,
                args0: [
                    {
                        type: 'field_image',
                        src: BEMFA_ICO,
                        width: 40,
                        height: 40
                    }
                ],
                args1: [
                    {
                        type: 'field_dropdown',
                        name: 'type',
                        options:[["TCP创客云","1"], ["TCP设备云","2"], ["图云","4"]]
                    },
                    {
                      type: "field_input",
                      name: "Key",
                      "text": "d9efdd0413ec4b74ab0057a0b8675654"
                    },
                    {
                      type: "field_input",
                      name: "topic",
                      "text": "led002"
                    },
                    {
                      type: "input_dummy"
                    },
                    {
                      type: "input_statement",
                      name: "function"
                    }
                ],
                "tooltip": "巴法云主题订阅",
                colour:BEMFA_COLOR,
                extensions: ['shape_statement']
            });
        }
    };

    //巴法云主题消息发送
    Blockly.Blocks.tcp_device_cloud_theme_push = {
        init: function () {
            this.jsonInit({
                message0: '%1',
                message1: Blockly.Msg.tcp_device_cloud_theme_push,
                args0: [
                    {
                        type: 'field_image',
                        src: BEMFA_ICO,
                        width: 40,
                        height: 40
                    }
                ],
                args1: [
                    {
                        type: 'field_dropdown',
                        name: 'type',
                        options:[["TCP创客云","1"], ["TCP设备云","2"], ["图云","4"]]
                    },
                    {
                      type: "field_input",
                      name: "Key",
                      "text": "d9efdd0413ec4b74ab0057a0b8675654"
                    },
                    {
                      type: "field_input",
                      name: "topic",
                      "text": "led002"
                    },
                    {
                      type: "input_value",
                      name: "data"
                    }
                ],
                "tooltip": "巴法云主题消息发送",
                colour:BEMFA_COLOR,
                extensions: ['shape_statement']
            });
        }
    };

    //串口打印
    Blockly.Blocks.serialPrint = {
        init: function () {
            this.jsonInit({
                message0: Blockly.Msg.serialPrint,
                args0: [
                    {
                        type: 'field_dropdown',
                        name: 'type',
                        options:[[(Blockly.Msg.println),"println"], [(Blockly.Msg.print),"print"]]
                    },
                    {
                        type: 'input_value',
                        name: 'VALUE'
                    }
                ],
                "tooltip": "串口打印",
                colour:BEMFA_COLOR,
                extensions: ['shape_statement']
            });
        }
    };

    //变量get
    Blockly.Blocks.variables_get = {
        init: function () {
            this.jsonInit({
                message0: '%1',
                args0: [
                    {
                        type: 'input_value',
                        name: 'VAR'
                    } 
                ],
                "tooltip": "获取变量",
                colour:BEMFA_COLOR,
                extensions: ['output_number']
            });
        }     
    };

    return Blockly;
}

exports = addBlocks;




