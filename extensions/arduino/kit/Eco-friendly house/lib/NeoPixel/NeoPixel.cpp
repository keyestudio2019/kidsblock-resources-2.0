...
NeoPixel::NeoPixel() : begun(false), pixels(NULL), pixels1(NULL), endTime(0)
{
...
void NeoPixel::begin(uint8_t p, uint16_t n, uint8_t bright, neoPixelType t)
{
    if (pin != -1)
    {
...
void NeoPixel::setPixelColor(
    uint16_t n, uint8_t g, uint8_t r, uint8_t b)
{
...
void NeoPixel::setPixelColor(
    uint16_t n, uint8_t g, uint8_t r, uint8_t b, uint8_t w)
{
...
void NeoPixel::rotate(int8_t offset)
{
...
void NeoPixel::showBarGraph(uint16_t start, uint16_t end, int16_t value, int16_t high)
{
    if (end < start)
    {
        uint16_t num = end;
...
void NeoPixel::shift(int8_t offset)
{
...
void NeoPixel::showRainbow(uint16_t start, uint16_t end, uint32_t startHue, uint32_t endHue)
{
...
uint32_t NeoPixel::hsl(uint32_t h, uint32_t s, uint32_t l)
{
    h = h % 360;
    s = max(s, 0);
    s = min(s, 99);
...
void NeoPixel::setRangeColor(int16_t start, int16_t end, uint32_t c)
{
...
void NeoPixel::setPixelColor(uint16_t n, uint32_t c, bool user)
{
...
void NeoPixel::setBrightness(uint32_t b)
{
...
uint32_t NeoPixel::rgbToColor(uint8_t r, uint8_t g, uint8_t b)
{
    uint32_t cr = r;
    uint32_t cg = g;
    uint32_t cb = b;
